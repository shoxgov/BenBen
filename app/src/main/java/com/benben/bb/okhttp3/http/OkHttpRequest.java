...
public class OkHttpRequest {
    ...
    public static void doExecute(final Request request, HttpCallback callback) {
        doExecute(request, BaseResponse.class, callback);
    }

    public static void doExecute(final Request request, final Class<BaseResponse> baseResponseClass, HttpCallback callback) {
        try {
            Response response = getOkHttpClient().newCall(request).execute();
            if (response.isSuccessful()) {
                OkHttpUtils.sendSuccessResultCallback(DataAnalysis.getReturnData(response.body().string(), baseResponseClass), callback);
            } else {
    ...
    public static void doEnqueue(final Request request, final HttpCallback callback) {
        doEnqueue(request, BaseResponse.class, callback);
    }

    public static void doEnqueue(final Request request, final Class baseResponseClass, final HttpCallback callback) {
        ...
        getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                ...
                try {
                    ...
                    obj.setCode(0);
                    obj.setMessage(file.getAbsolutePath());
                    OkHttpUtils.sendSuccessResultCallback(obj, callback);
                } catch (IOException e) {
                    e.printStackTrace();
                    OkHttpUtils.sendFailResultCallback(-1, e.getMessage(), callback);
                } finally {
